...
def parse(String description) {
	...
	if (descMap && descMap.clusterInt == 0x0008) {
		...
		case 0x04:
			if (descMap.data[0] == "00") {
                ...
                resultDir = createEvent(name: "goal", value: 1)
                resultTime = createEvent(name: "steps", value: 0)
			} else {
				log.debug "Fast turn cw, value to 100"
				result = createEvent(name: "level", value: 100)
                resultDir = createEvent(name: "goal", value: 0)
                resultTime = createEvent(name: "steps", value: 0)
			}
			break
		case 0x05:
		...
		case 0x07:
            ...
            resultDir = createEvent(name: "goal", value: state.direction)
            resultTime = createEvent(name: "steps", value: iTime)
	...
	return [result,resultDir,resultTime]
}
